...
import { paymentStatusPillClass } from '../utils/paymentStatusFilter'

type OrderRowPaymentStatusSelectProps = {
  ...
  locked?: boolean
  saving?: boolean
  onCommit: (orderId: string, paymentStatus: string) => Promise<void>
  triggerClassName?: string
}

export function OrderRowPaymentStatusSelect({
  orderId,
  value,
  options,
  getOptionLabel,
  ...
  onCommit,
  triggerClassName,
}: OrderRowPaymentStatusSelectProps) {
  ...
  const pill = paymentStatusPillClass(displayCode)
                ...
                <li key={code} role="option" aria-selected={active}>
                  <button
                    ...
                    className={cn(
                      ...
                      active && 'bg-emerald-50 font-medium dark:bg-emerald-900/30',
                    )}
                    onClick={(e) => {
                      e.stopPropagation()
                      void onPick(code)
  ...
  return (
    <div className="inline-block text-left" onClick={(e) => e.stopPropagation()}>
      <button
        ...
        aria-expanded={open}
        aria-label={t('ordersPage.changePaymentStatusAria')}
        onClick={(e) => {
          e.stopPropagation()
          if (!disabled) setOpen((v) => !v)
        }}
        className={cn(
          'inline-flex max-w-full items-center gap-1 rounded-full border border-transparent px-2 py-1 text-xs font-semibold whitespace-nowrap transition hover:ring-2 hover:ring-emerald-500/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/70 disabled:cursor-not-allowed disabled:opacity-60',
          pill,
          triggerClassName,
        )}
      >
